...
namespace TesPGP
{
    public static class CryptoHelper
    {
        ...
        public static readonly string PublicKey = "PgpPublicKey.asc";
        ...
        private static readonly string PrivateKeyOnly = "PgpPrivateKeyOnly.asc";
        ...
        public static string DecryptPgpFile(string inputFile)
        {
            string output;
            ...
            using (Stream inputStream = IoHelper.GetStream(inputData))
            {
                using (Stream keyIn = File.OpenRead(PrivateKeyOnlyPath))
                {
                    output = DecryptPgpFile(inputStream, keyIn, Password);
                }
            }
            return output;
        }
        public static string DecryptPgpFile(Stream inputStream, Stream privateKeyStream, string passPhrase)
        {
            string output;
            ...
            if (message is PgpCompressedData)
            {
                ...
                using (Stream unc = literalData.GetInputStream())
                {
                    using (var fileStream = new FileStream(IoHelper.BasePath + @"\pgp-decrypt.txt", FileMode.Create, FileAccess.Write))
                    {
                        unc.CopyTo(fileStream);
                    }
